...
const HeroSection = () => (
    <div className="relative h-[30vh] w-full bg-gray-900 overflow-hidden mb-8">
        ...
        <img src="https://images.unsplash.com/photo-1574267432553-4b4628081c31?q=80&w=2000&auto=format&fit=crop" className="w-full h-full object-cover opacity-50" alt="Hero"/>
        <div className="absolute bottom-0 left-0 p-8 z-20">
            <h1 className="text-4xl font-bold text-white mb-2">Explore & Discover</h1>
            <p className="text-gray-300">Dive into the universe of entertainment.</p>
        </div>
    </div>
);
...
export default function App() {
  ...
  useEffect(() => {
      fetchMovies(1, false);
  }, [selectedCategory, comingFilter, selectedRegion, filterPeriod, selectedLanguage, sortOption, activeCountry, activeKeyword, tmdbCollectionId, userProfile.age, searchQuery]);
  ...
  const fetchMovies = useCallback(async (pageNum: number = 1, isLoadMore = false) => {
    ...
    try {
        ...
        const isGeneralDiscovery = !activeCountry && !activeKeyword && !tmdbCollectionId && !["People", "Franchise"].includes(selectedCategory) && !searchQuery;
