    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {credits.map((p, idx) => (
                    <div key={idx} onClick={() => { onClose(); onPersonClick(p.id); }} className="p-4 bg-white/5 rounded-xl text-center cursor-pointer hover:bg-white/10 transition-colors group">
                        <img src={p.profile_path ? `${TMDB_IMAGE_BASE}${p.profile_path}` : `https://ui-avatars.com/api/?name=${p.name}`} className="w-20 h-20 rounded-full mx-auto mb-3 object-cover shadow-lg group-hover:scale-105 transition-transform" alt=""/>
...
export const PersonPage: React.FC<{ personId: number; onClose: () => void; apiKey: string; onMovieClick: (m: Movie) => void; }> = ({ personId, onClose, apiKey, onMovieClick }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[#0a0a0a] overflow-y-auto animate-in slide-in-from-bottom-10">
            <button onClick={onClose} className="fixed top-6 left-6 z-[120] bg-black/40 px-4 py-2 rounded-full text-white flex items-center gap-2 border border-white/5 hover:bg-white/10 transition-all backdrop-blur-md"><ArrowLeft size={20}/> Back</button>
            {loading ? <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-red-600" size={40}/></div> : details && (
                <div className="flex flex-col lg:flex-row min-h-screen">
                    <div className="w-full lg:w-80 shrink-0 bg-black/40 p-10 border-r border-white/5 flex flex-col items-center">
                        ...
                        <h2 className="text-2xl font-bold text-white text-center">{details.name}</h2>
                    ...
                    <div className="flex-1 p-10">
                        ...
                        <p className="text-gray-400 text-sm leading-relaxed mb-10 whitespace-pre-line font-light">{details.biography || "No biography available for this artist."}</p>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            {details.combined_credits?.cast?.sort((a:any, b:any) => b.popularity - a.popularity).slice(0, 10).map((m: any) => (
                                <div key={m.id} onClick={() => onMovieClick(m)} className="cursor-pointer group">
                                    <img src={m.poster_path ? `${TMDB_IMAGE_BASE}${m.poster_path}` : ""} className="aspect-[2/3] rounded-lg mb-2 group-hover:scale-105 transition-all shadow-lg" alt=""/>
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-[#121212] border border-white/10 rounded-3xl p-8 w-full max-w-md text-center">
                ...
                <button onClick={onClose} className="w-full bg-white/5 py-3 rounded-xl font-bold text-white hover:bg-white/10 transition-all active:scale-95">Close</button>
            </div>
        </div>
    );
};
...
export const ComparisonModal: React.FC<{ isOpen: boolean; onClose: () => void; baseMovie: Movie | null; onMovieSelect: (m: Movie) => void; }> = ({ isOpen, onClose, baseMovie, onMovieSelect }) => {
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-[#121212] border border-white/10 rounded-3xl p-8 w-full max-w-4xl max-h-[80vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <img src={baseMovie.poster_path ? `${TMDB_IMAGE_BASE}${baseMovie.poster_path}` : "https://placehold.co/300x450"} className="w-full rounded-2xl shadow-2xl" alt=""/>
                        <h3 className="text-xl font-bold text-white">{baseMovie.title}</h3>
                        <p className="text-sm text-gray-400 line-clamp-3 font-light">{baseMovie.overview}</p>
                    </div>
                    <div className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-2xl p-8 text-gray-500 bg-white/[0.02]">
