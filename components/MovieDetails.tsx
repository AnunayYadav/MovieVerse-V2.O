...
export const MovieModal: React.FC<MovieModalProps> = ({ 
    ...
    if (isPlaying) {
        return (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-2xl animate-in fade-in duration-300">
                <div className="w-full h-full md:h-auto md:aspect-video md:max-w-6xl bg-black md:rounded-2xl overflow-hidden shadow-2xl border border-white/10 relative flex flex-col">
                    <MoviePlayer 
                        tmdbId={movie.id} 
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-end md:items-center justify-center md:p-6 animate-in fade-in duration-200">
            {/* Backdrop with Blur */}
            ...
            {/* Modal Content */}
            <div className="glass-panel w-full md:max-w-5xl md:rounded-3xl rounded-t-3xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col z-10">
                <button onClick={onClose} className="absolute top-4 right-4 z-20 bg-black/40 hover:bg-white/10 backdrop-blur-md p-2 rounded-full text-white/80 hover:text-white transition-colors border border-white/5"><X size={20} /></button>
                        ...
                        <div className="relative h-[60vh] md:h-[500px] w-full shrink-0">
                             ...
                             <div className="absolute bottom-0 left-0 p-6 md:p-10 w-full md:w-2/3 flex flex-col gap-4">
                                ...
                                <div className="flex flex-wrap gap-3 mt-2">
                                    <button onClick={() => setIsPlaying({ season: 1, episode: 1 })} className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-red-900/20 active:scale-95"><PlayCircle size={20} fill="currentColor" /> {isTv ? "Start Watching" : "Watch Now"}</button>
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => onToggleWatchlist(displayData)} className={`p-3 rounded-xl border transition-colors ${isWatchlisted ? 'bg-red-600/20 border-red-500/50 text-red-400' : 'glass hover:bg-white/10 text-white/70'}`}><Bookmark size={20} fill={isWatchlisted ? "currentColor" : "none"} /></button>
                                        <button onClick={() => onToggleFavorite(displayData)} className={`p-3 rounded-xl border transition-colors ${isFavorite ? 'bg-red-600/20 border-red-500/50 text-red-400' : 'glass hover:bg-white/10 text-white/70'}`}><Heart size={20} fill={isFavorite ? "currentColor" : "none"} /></button>
                                        <button onClick={() => onOpenListModal(displayData)} className="p-3 rounded-xl glass hover:bg-white/10 text-white/70 transition-colors" title="Add to Custom List"><ListPlus size={20} /></button>
                                        <button onClick={handleShare} className={`p-3 rounded-xl glass hover:bg-white/10 transition-colors ${copied ? 'text-green-400' : 'text-white/70'}`}>{copied ? <Check size={20} /> : <Share2 size={20} />}</button>
                                    </div>
                                </div>
                             </div>
                        </div>
                        ...
                        <div className="p-6 md:p-10 flex flex-col gap-8">
                            ...
                            <div>
                                <div className="flex gap-8 border-b border-white/10 mb-6">
                                    <button onClick={() => setActiveTab("details")} className={`pb-3 text-sm font-bold tracking-wide transition-colors ${activeTab === "details" ? "text-red-500 border-b-2 border-red-500" : "text-white/50 hover:text-white"}`}>DETAILS</button>
                                    {(isTv || isAnime) && <button onClick={() => setActiveTab("episodes")} className={`pb-3 text-sm font-bold tracking-wide transition-colors ${activeTab === "episodes" ? "text-red-500 border-b-2 border-red-500" : "text-white/50 hover:text-white"}`}>EPISODES</button>}
                                    <button onClick={() => setActiveTab("media")} className={`pb-3 text-sm font-bold tracking-wide transition-colors ${activeTab === "media" ? "text-red-500 border-b-2 border-red-500" : "text-white/50 hover:text-white"}`}>MEDIA</button>
                                </div>
                                ...
                                {activeTab === "details" ? (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                                        {providers.length > 0 && (
                                            <div className="bg-gradient-to-br from-blue-900/10 to-transparent p-5 rounded-2xl border border-white/5">
                                                ...
                                                <div className="flex flex-wrap gap-3">{providers.map((p, i) => ( <a key={i} href={`https://www.google.com/search?q=watch+${encodeURIComponent(title)}+on+${encodeURIComponent(p.provider_name)}`} target="_blank" rel="noreferrer" className={`glass px-3 py-2 rounded-lg flex items-center gap-2 hover:bg-white/10 transition-colors group`}><img src={`${TMDB_IMAGE_BASE}${p.logo_path}`} alt={p.provider_name} className="w-6 h-6 rounded-md shadow-sm" /><span className="text-xs font-bold text-gray-300 group-hover:text-white">{p.provider_name}</span></a> ))}</div>
                                            </div>
                                        )}
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div className="glass p-5 rounded-2xl">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {cast.map((c, i) => (
                                                        <button key={i} onClick={() => onPersonClick(c.id)} className="text-xs font-medium bg-white/5 hover:bg-white/10 border border-white/5 px-3 py-1.5 rounded-full text-gray-300 hover:text-white transition-colors">
                                                            {c.name}
                                                        </button>
                                                    ))}
                                                </div>
                                    ...
                                    <div className="animate-in fade-in slide-in-from-right-2 min-h-[300px]">
                                                ...
                                                {seasonData.episodes?.map(ep => (
                                                    <div key={ep.id} className="group flex gap-4 p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all cursor-pointer" onClick={() => setIsPlaying({ season: selectedSeason, episode: ep.episode_number })}>
                                                        <div className="relative w-32 md:w-40 aspect-video shrink-0 rounded-lg overflow-hidden bg-white/5">
                                                            <img src={ep.still_path ? `${TMDB_IMAGE_BASE}${ep.still_path}` : "https://placehold.co/300x170/111/333"} className="w-full h-full object-cover group-hover:scale-105 transition-transform opacity-70 group-hover:opacity-100" alt={ep.name}/>
                                                            <div className="absolute inset-0 flex items-center justify-center"><PlayCircle size={24} className="text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg"/></div>
                                                        </div>
                                    ...
                                    <div className="space-y-6">
                                        {mediaImages.length > 0 && <div><h4 className="text-white/60 font-bold text-xs uppercase mb-3 tracking-wider">Gallery</h4><div className="grid grid-cols-2 md:grid-cols-4 gap-3">{mediaImages.map((img, idx) => (<div key={idx} className="aspect-video rounded-lg overflow-hidden cursor-pointer relative group" onClick={() => setViewingImage(`${TMDB_BACKDROP_BASE}${img.file_path}`)}><img src={`${TMDB_IMAGE_BASE}${img.file_path}`} alt="Scene" className="w-full h-full object-cover group-hover:scale-110 transition-transform opacity-80 group-hover:opacity-100"/><div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors"/></div>))}</div></div>}
                                        {videoClips.length > 0 && <div><h4 className="text-white/60 font-bold text-xs uppercase mb-3 tracking-wider">Videos</h4><div className="space-y-2">{videoClips.map((v, idx) => ( <div key={idx} className="flex gap-4 p-3 rounded-xl hover:bg-white/5 cursor-pointer group" onClick={() => window.open(`https://www.youtube.com/watch?v=${v.key}`, '_blank')}><div className="relative w-32 aspect-video bg-black rounded-lg overflow-hidden shrink-0"><img src={`https://img.youtube.com/vi/${v.key}/mqdefault.jpg`} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-opacity"/><div className="absolute inset-0 flex items-center justify-center"><Play size={20} className="text-white fill-white"/></div></div><div><p className="text-white font-bold text-sm line-clamp-1">{v.name}</p><p className="text-xs text-white/40 mt-1">{v.type}</p></div></div> ))}</div></div>}
                                    </div>
                                )}
                            </div>
                                    ...
                                    <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                                        {similarMovies.map(m => (
                                            <div key={m.id} className="cursor-pointer group relative aspect-[2/3] rounded-lg overflow-hidden" onClick={() => onSwitchMovie(m)}>
                                                <img src={m.poster_path ? `${TMDB_IMAGE_BASE}${m.poster_path}` : "https://placehold.co/100x150"} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt={m.title}/>
