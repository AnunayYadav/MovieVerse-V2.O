      ...
      <div className="relative w-full overflow-hidden py-8 border-y border-white/5 bg-black/40 backdrop-blur-sm mb-8 transition-all duration-500">
        ...
        <div className="flex animate-marquee hover:[animation-play-state:paused]" style={{ width: 'max-content' }}>
          {marqueeMovies.map((movie, index) => (
             <button 
               key={`${movie.id}-${index}`} 
               className="flex-shrink-0 w-32 md:w-48 mx-3 cursor-pointer group relative transition-all duration-500 ease-out hover:scale-105 hover:z-10 focus:outline-none"
               onClick={() => onMovieClick(movie)}
             >
                <div className="aspect-[2/3] rounded-xl overflow-hidden shadow-2xl group-hover:shadow-[0_0_25px_rgba(220,38,38,0.3)] transition-all duration-500 group-focus:shadow-[0_0_25px_rgba(220,38,38,0.3)]">
                    <img 
                      src={movie.poster_path ? `${TMDB_IMAGE_BASE}${movie.poster_path}` : (movie.poster || "https://placehold.co/300x450/333/FFF?text=Movie")} 
                      alt={movie.title || "Movie"} 
                      loading="lazy"
                      className="w-full h-full object-cover opacity-60 grayscale group-hover:grayscale-0 group-hover:opacity-100 group-focus:grayscale-0 group-focus:opacity-100 transition-all duration-500"
                    />
                </div>
             </button>
          ))}
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-300 font-sans" onClick={onClose}>
         <button 
           ...
           className="absolute top-6 left-6 text-white hover:text-red-500 bg-white/5 hover:bg-white/10 p-3 rounded-full transition-all hover:scale-110 active:scale-95 flex items-center gap-2 group focus:outline-none"
         >
           <ArrowLeft size={24}/>
         ...
         >
            ...
            <button onClick={handleDownload} className="mt-6 glass px-8 py-3 rounded-full font-bold text-sm flex items-center gap-2 hover:bg-white/20 transition-all text-white active:scale-95 focus:outline-none">
...
export const MovieCard = React.forwardRef<HTMLButtonElement, MovieCardProps>(({ movie, onClick, isWatched, onToggleWatched }, ref) => {
    ...
    return (
      <button 
        ref={ref}
        className="group relative rounded-xl overflow-hidden cursor-pointer transition-all duration-500 ease-[cubic-bezier(0.33,1,0.68,1)] hover:z-20 hover:scale-[1.03] hover:shadow-[0_10px_40px_rgba(0,0,0,0.5)] focus:scale-[1.05] focus:outline-none focus:shadow-[0_0_20px_#ef4444] font-sans text-left w-full"
      ...
      >
        <div className="aspect-[2/3] overflow-hidden bg-white/5 relative">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-60 group-hover:opacity-90 group-focus:opacity-90 transition-opacity duration-500 pointer-events-none" />
          
          {/* Content Overlay */}
          <div className="absolute inset-0 p-4 pb-6 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 group-focus:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.33,1,0.68,1)] pointer-events-none">
             <div className="flex flex-col gap-1 mb-2">
                ...
                <h3 className="text-white font-bold text-base leading-tight drop-shadow-md line-clamp-2 transform translate-y-2 group-hover:translate-y-0 group-focus:translate-y-0 transition-transform duration-500 delay-75">{movie.title || movie.name}</h3>
                <div className="flex items-center justify-between text-white/70 text-xs transform translate-y-2 group-hover:translate-y-0 group-focus:translate-y-0 transition-transform duration-500 delay-100">
                  <span>{year || 'TBA'}</span>
        ...
        {/* Action Buttons */}
        <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0 group-focus:translate-x-0 z-30">
             <button 
               onClick={(e) => { e.stopPropagation(); onToggleWatched(movie); }}
               className={`p-2 rounded-full backdrop-blur-md shadow-lg transition-all hover:scale-110 active:scale-95 focus:outline-none ${isWatched ? 'text-green-400 bg-black/60' : 'text-white/80 bg-black/40 hover:bg-white hover:text-black focus:bg-white focus:text-black'}`}
...
export const PersonCard = React.forwardRef<HTMLButtonElement, PersonCardProps>(({ person, onClick }, ref) => {
