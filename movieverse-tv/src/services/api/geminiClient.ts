...
export const getGeminiKey = async (): Promise<string> => {
  try {
    const localKey = await AsyncStorage.getItem('movieverse_gemini_key');
    if (localKey) return localKey;
  } catch (e) {}
  
  // Return compilation environment variable if defined, or placeholder
...
export const generateTrivia = async (movieTitle: string, year: string): Promise<string> => {
  try {
      const apiKey = await getGeminiKey();
      if (!apiKey) return "Trivia unavailable (Key not configured).";
...
export const generateSmartRecommendations = async (query: string): Promise<{ movies: string[], reason: string }> => {
  try {
      const apiKey = await getGeminiKey();
      if (!apiKey) return { movies: [], reason: "Gemini API key is not configured." };
